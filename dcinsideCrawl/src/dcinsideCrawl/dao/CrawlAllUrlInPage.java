...
public class CrawlAllUrlInPage {
	...
	public ArrayList<String> CAUIP( WebDriver wd , String age_range ) throws InterruptedException {
		...
		try { sc.searchEachAge( age_range , wd ) ; } catch (Exception e) { System.out.println( "sc.searchEachAge( age_range , wd )에서 에러발생" ); }
		
		
		for ( int i = 1 ; i < 10 ; i++ ) {
			
			urlListInPage = (ArrayList<WebElement>) wd.findElements(By.cssSelector(Config.cf.EURLSelctor));
