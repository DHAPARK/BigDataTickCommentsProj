package fmkoreaCrawl.EachAgeCrawlThread;
...
import fmkoreaCrawl.dao.CrawlAllUrlInPage;
import fmkoreaCrawl.vo.ChromeOpt;
import fmkoreaCrawl.vo.Config;

public class Get40SC extends CrawlThread {

	@Override
	public void run() {
		
		getTermBtnECO( Config.cf.getTermBtnECO_Time * 4 );
		
		this.copt = new ChromeOpt( Config.cf.fmkorea_Age_40_URL );
		...
		try {
			 ...
			 */
			allUrlList = cauip.CAUIP( wd , Config.cf.Age_40_Keyword );
			...
			for (String item : allUrlList) {
				...
				this.splitKWIC.StoreSplitedKWIC( Config.cf.Age_40 , this.splitKWIC.SplitKWIC( commentsList ) ) ; 
